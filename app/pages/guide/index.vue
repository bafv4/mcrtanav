<template>
  <div class="head">
    <p class="th-1">ガイド</p>
    <p class="text-body-2 text-medium-emphasis mb-4">マインクラフトRTAに関するガイドやチュートリアルを掲載しています。</p>
  </div>
  ...
  <div v-else-if="files.length === 0" class="text-center pa-8">
    ...
    <p class="text-body-1 text-medium-emphasis">ガイドはありません。</p>
  </div>
  ...
  <div v-else class="d-flex flex-column ga-3">
    <v-card v-for="(file, i) in files" :key="i" :to="`/${file.slug}`" variant="outlined" hover class="cursor-pointer">
        ...
        <div class="d-flex align-start justify-space-between">
          <div class="flex-grow-1">
            ...
            <div class="d-flex align-center flex-wrap ga-2">
              <!-- タグ表示 -->
              <v-chip v-for="tag in file.meta.tags" :key="tag" size="small" variant="tonal" color="primary">
                {{ tag }}
              </v-chip>
      ...
      <v-card-actions class="pt-0">
        <v-btn variant="text" color="primary" append-icon="mdi-arrow-right" size="small">
          読む
        </v-btn>
      </v-card-actions>
    </v-card>
...
const { fetchFilesWithMeta } = useMarkdown()
const files = ref<any[]>([]);
...
onMounted(async () => {
  try {
    files.value = await fetchFilesWithMeta('guide');
    console.log('Loaded files with metadata:', files.value)
  } catch (error) {
